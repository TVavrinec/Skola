...
int compareAlbums(album_t *a, album_t *b, int compare_faktor)
{
	...
	case albumName:
		...
		return (a->name[i] < b->name[i]) ? 1 : 0;
		break;
	case albumInterpreter:
		while(a->interpreter[i] == b->interpreter[i])i++;
		return (a->interpreter[i] < b->interpreter[i]) ? 1 : 0;
		break;
	case albumYear:
	...
	case albumGenre:
		...
		return (a->genre[i] < b->genre[i]) ? 1 : 0;
		break;
	case albumScore:
...
album_t *addNewAlbum(album_t *prev, char name[20], char interpreter[20], int year, char genre[20], float score)
{
	album_t *new_album = (album_t *)malloc(sizeof(album_t));
	new_album->next = NULL;
...
album_t *sortAlbums(album_t *first_album, int compare_faktor, bool sort_dir)
{
	...
	{
		switch_count = 0;
		first_album = (compareAlbums(first_album,first_album->next, compare_faktor)==sort_dir) ? switchAlbums(first_album, first_album->next)->prev : first_album;
		album = first_album->next;
		while(album->next!=NULL)
		{
			if(compareAlbums(album,album->next, compare_faktor)==sort_dir)
			{
