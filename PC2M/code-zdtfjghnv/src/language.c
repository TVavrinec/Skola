...
{
    return (isOperator(x)||isNumber(x)||(x==')')||(x=='(')||(x=='_')) ? true : false;
}
...
bool isSymbolValid(char x)
{
    ...
    return (isNumber(x)||isOperator(x)||isMathSim(x)||isLatter(x)||(x==' ')||(x=='-')||(x==',')) ? true : false;
}
...
readed_number_t *addMember(readed_number_t *list, char *member)
{
    readed_number_t *prev = list;
    readed_number_t *new_member = (readed_number_t *)malloc(sizeof(readed_number_t));
    *new_member = loadNumber(list);
    new_member->next = NULL;
    new_member->marker = member[new_member->length];
    if(!(isOperator(new_member->marker)||(new_member->marker==')'))) sintaxError(member);
    new_member->length++;
    if(prev != NULL)
        for (int a = 0; prev->next != NULL; a++) prev = prev->next;
    prev->next = new_member;
    return new_member;
}

readed_number_t *makeArithmeticsList(readed_number_t *list,char *buffer)
{
    if((buffer[0] == ';')||(buffer[0] == ')')) return NULL;
    readed_number_t number;
    if(isNumber(buffer[0])||(buffer[0]=='-')) number = *addMember(list, buffer);
    else if(buffer[0] == '(') number =
